...
public class NumberRangeControl extends Control {
    ...
    private final StyleableObjectProperty<Color> outOfRangeColor = new SimpleStyleableObjectProperty<Color>(OUT_OF_RANGE_COLOR_META_DATA, this, "outOfRangeColor") {
        @Override
        protected void invalidated() {
            setStyle(OUT_OF_RANGE_COLOR_META_DATA.getProperty() + ": " + (getOutOfRangeColor()).toString().replace("0x", "#"));
            applyCss();
        }
    };
	...
	public double valueFromDegrees(double alpha) {
		return valueFromPercentage((alpha / 3.6) / 100.0);
	}
	 ...
	 * @return new value
	 */
	public double valueFromPercentage(double percentage) {
		return ((getMaxValue() - getMinValue()) * percentage) + getMinValue();
	}
	...
	private void addValueChangedListeners() {
		...
		outOfRangeProperty().addListener((observable, oldValue, newValue) -> {
			...
			if (newValue) {
				setBaseColor(getOutOfRangeColor());
			} else {
