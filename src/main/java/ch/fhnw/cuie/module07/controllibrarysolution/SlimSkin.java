...
class SlimSkin extends SkinBase<NumberRangeControl> {
	...
	SlimSkin(NumberRangeControl control) {
		...
		Platform.runLater(() -> {
			relocateTexts();
			relocateThumb(getSkinnable().getValue());
		});
	}
	...
	private void initializeParts() {
		...
		valueLabel.setY(CIRCLE_CENTER_Y);
	...
	private void addEventHandlers() {
		thumb.setOnMouseDragged(event -> {
			double a = event.getX() - CIRCLE_CENTER_X;
			double b = CIRCLE_CENTER_Y - event.getY();
			...
			if (a > 0 && b > 0) {
				...
				alpha = 90 + Math.toDegrees(Math.asin(-b / c));
			} else if (a <= 0 && b <= 0) {
				alpha = 180 + Math.toDegrees(Math.asin(-a / c));
			} else {
				alpha = 270 + Math.toDegrees(Math.asin(b / c));
			}

			getSkinnable().setAnimated(false);
			getSkinnable().setValue(getSkinnable().valueFromDegrees(alpha));
	...
	private void addValueChangedListeners() {
		...
		getSkinnable().angleProperty().addListener((observable, oldValue, newValue) -> {
			relocateThumb(newValue);
		});
	...
	private void relocateThumb(Number newValue) {
		double alpha = Math.toRadians(-1.0 * newValue.doubleValue() - START_ANGLE);

		thumb.setCenterX(RADIUS * Math.cos(alpha) + CIRCLE_CENTER_X);
		thumb.setCenterY(RADIUS * Math.sin(alpha) + CIRCLE_CENTER_Y);
	}
